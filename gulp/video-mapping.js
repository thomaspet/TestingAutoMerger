...
gulp.task('video-mapping', function(done) {
    if (!process.env.GITHUB_PERSONAL_ACCESS_TOKEN || !process.env.GITHUB_USERNAME) {
        console.log(
            'Cound not find the GITHUB_USERNAME and GITHUB_PERSONAL_ACCESS_TOKEN environment keys, '
            + 'skipping video-mapping.');
        return done();
    }
    ...
    req.on('response', function(response) {
        if (response.statusCode < 200 || response.statusCode > 300) {
            done(new Error('Got a non 2XX response ('
                + response.statusCode
                + ') when trying to download video mapping JSON file.'));
        }
        ...
        response.on('end', () => {
            ...
            fs.writeFile(OUTPUT_FILE, JSON.stringify(mappingDictionary), done);
        });
    });

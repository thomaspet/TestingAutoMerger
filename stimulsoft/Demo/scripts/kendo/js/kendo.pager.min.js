...
!function (e, define) {
    ...
    return function (e, t) {
        ...
        var c = window.kendo, d = c.ui, h = d.Widget, u = e.proxy, f = ".k-i-seek-w", p = ".k-i-seek-e", g = ".k-i-arrow-w", m = ".k-i-arrow-e", v = "change", _ = ".kendoPager", w = "click", y = "keydown", b = "disabled", x = c.template('<a href="\\#" title="#=text#" class="k-link k-pager-nav #= wrapClassName #"><span class="k-icon #= className #">#=text#</span></a>'), k = h.extend({
            ...
            destroy: function () {
                var e = this;
                h.fn.destroy.call(e), e.element.off(_), e.dataSource.unbind(v, e._refreshHandler), e._refreshHandler = null, c.destroy(e.element), e.element = e.list = null
            },
            events: [v],
            options: {
                name: "Pager",
            ...
            refresh: function (e) {
                var t, i, o, d, h, u, f = this, p = 1, g = f.page(), m = "", v = f.options, _ = f.pageSize(), w = f.dataSource.total(), y = f.totalPages(), x = f.linkTemplate, k = v.buttonCount;
                if (!e || "itemchange" != e.action) {
                    if (v.numeric) {
                        for (g > k && (o = g % k, p = 0 === o ? g - k + 1 : g - o + 1), i = Math.min(p + k - 1, y), p > 1 && (m += n(x, p - 1, "...", !1, v.messages.morePages)), t = p; i >= t; t++)m += n(t == g ? f.selectTemplate : x, t, t, !0);
                        y > i && (m += n(x, t, "...", !1, v.messages.morePages)), "" === m && (m = f.selectTemplate({text: 0})), m = this.currentPageTemplate({text: g}) + m, f.list.removeClass("k-state-expanded").html(m)
                    }
                    v.info && (m = w > 0 ? c.format(v.messages.display, (g - 1) * _ + 1, Math.min(g * _, w), w) : v.messages.empty, f.element.find(".k-pager-info").html(m)), v.input && f.element.find(".k-pager-input").html(f.options.messages.page + '<input class="k-textbox">' + c.format(v.messages.of, y)).find("input").val(g).attr(b, 1 > w).toggleClass("k-state-disabled", 1 > w), v.previousNext && (r(f.element, g, y), a(f.element, g, y), s(f.element, g, y), l(f.element, g, y)), v.pageSizes && (d = f.element.find(".k-pager-sizes option[value='all']").length > 0, h = d && _ === this.dataSource.total(), u = _, h && (_ = "all", u = v.messages.allPages), f.element.find(".k-pager-sizes select").val(_).filter("[" + c.attr("role") + "=dropdownlist]").kendoDropDownList("value", _).kendoDropDownList("text", u))
                }
            },
            _keydown: function (e) {
            ...
            _change: function (e) {
                var t = e.currentTarget.value, n = parseInt(t, 10), i = this.dataSource;

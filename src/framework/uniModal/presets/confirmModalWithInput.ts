import {Component, Input, Output, EventEmitter} from '@angular/core';
import {IUniModal, IModalOptions, ConfirmActions} from '@uni-framework/uniModal/interfaces';

@Component({
    selector: 'uni-confirm-modal-with-input',
    template: `
        <section role="dialog" class="uni-modal">
            <header>
                <h1 class="new">{{options.header}}</h1>
            </header>

            <article>
                <section [innerHtml]="options.message"></section>
                <p class="warn" *ngIf="options.warning">
                    {{options.warning}}
                </p>
                <br>
                <p>Ny dato på det krediterte bilaget: </p>
                <input type="date" [(ngModel)]="input">
                <p *ngIf="requiredInput" style="color:red">Fyll ut dato</p>
            </article>

            <footer>
                <button *ngIf="options.buttonLabels.accept" class="good" id="good_button_ok" (click)="accept()">
                    {{options.buttonLabels.accept}}
                </button>

                <button *ngIf="options.buttonLabels.reject" class="bad" (click)="reject()">
                    {{options.buttonLabels.reject}}
                </button>

                <button *ngIf="options.buttonLabels.cancel" class="cancel" (click)="cancel()">
                    {{options.buttonLabels.cancel}}
                </button>
            </footer>
        </section>
    `
})
export class UniConfirmModalWithInput implements IUniModal {
    @Input()
    public options: IModalOptions = {};

    @Output()
    public onClose: EventEmitter<any> = new EventEmitter();

    public input: Date;
    public requiredInput: boolean;

    public ngOnInit() {
        if (!this.options.buttonLabels) {
            this.options.buttonLabels = {
                accept: 'Ok',
                cancel: 'Avbryt'
            };
        }
    }

    public ngAfterViewInit() {
        setTimeout(function() {
            if (document.getElementById('good_button_ok')) {
                document.getElementById('good_button_ok').focus();
            }
        });
    }

    public accept() {
        if (this.input) {
            this.onClose.emit({input: this.input, action: ConfirmActions.ACCEPT});
        }
        return this.requiredInput = true;
    }

    public reject() {
        this.onClose.emit({input: this.input, action: ConfirmActions.REJECT});
    }

    public cancel() {
        this.onClose.emit({input: this.input, action: ConfirmActions.CANCEL});
    }
}

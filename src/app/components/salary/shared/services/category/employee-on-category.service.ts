...
import {PayrollRunOnCategoryService} from '@app/components/salary/shared/services/category/payroll-run-on-category.service';
...
export class EmployeeOnCategoryService extends BizHttp<EmployeeCategory> {
    ...
    private askAndForceDeleteAll(categoryID: number, empIDs: number[]) {
        ...
        .pipe(
            ...
            switchMap(runs =>
                runs.length ? this.askIfTheUserWantsToForceDelete(runs) : Observable.of({action: ConfirmActions.ACCEPT, runs: null})),

...
import {
    ...
    SalarybalanceService, SalaryBalanceLineService, PageStateService
...
export class EmployeeDetailsComponent extends UniView implements OnDestroy {
    ...
    public canDeactivate(): Observable<boolean> {
        return of(!super.isDirty() || !!this.savedNewEmployee).pipe(
            switchMap(isSaved => isSaved ? of(ConfirmActions.ACCEPT) : this.modalService.openUnsavedChangesModal().onClose),
            switchMap((action: ConfirmActions) => Observable.forkJoin(this.createSaveObjects(), Observable.of(action))),
            switchMap((result: [ISaveObject[], ConfirmActions]) => {
                const [saveObj, action] = result;
                return this.canNavigateAfterSave(action, saveObj);
            }),
            tap(x => {
                if (x) {
                    this.cacheService.clearPageCache(this.cacheKey);
    ...
    private canNavigateAfterSave(action: ConfirmActions, saveObj: ISaveObject[]): Observable<boolean> {
        ...
        return this.saveAllObs({ done: (m) => { }, ignoreRefresh: true }, saveObj).pipe(
            ...
            map((x: boolean) => {
                ...
                return !this.saveStatus.hasErrors;
            })
        );
    }
                ...
                (employee) => {
                    ...
                    return this.employeeService
                        ...
                        .switchMap((emp) => {
                            ...
                            return Observable.forkJoin([of(emp), ...obsList]);
                        });
                }
            )
            .pipe(

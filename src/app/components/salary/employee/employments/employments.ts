...
export class EmployeeEmployment {
    ...
    private buildFormConfigs(): void {
        ...
        this.currentEmployee.Employments.forEach((employment: Employment) => {
            ...
            jobCode.onSelect = (event: kendo.ui.AutoCompleteSelectEvent) => {
                ...
                var fjn = <UniFieldBuilder>UniElementFinder
                .findUniFieldByPropertyName('JobName', formbuilder.config());
            ...
            jobName.onSelect = (event: kendo.ui.AutoCompleteSelectEvent) => {
                var item: any = event.item;
                var dataItem = event.sender.dataItem(item.index());
                var fjc = <UniFieldBuilder>UniElementFinder
                .findUniFieldByPropertyName('JobCode', formbuilder.config());
            ...
            if (typeof employment.SubEntity) {
                if (typeof employment.SubEntity.BusinessRelationInfo) {
                    var subEntity = this
                        .buildField('Lokasjon', employment.SubEntity.BusinessRelationInfo, 'Name', 
                            FieldType.COMBOBOX);
            ...
            if (addSubEntity) {
                formbuilder.addUniElements(
                    jobCode, jobName, startDate, endDate, monthRate
                    , hourRate, workPercent, subEntity, readgroup
                    );
            } else {
                formbuilder.addUniElements(
                    jobCode, jobName, startDate, endDate, monthRate
                    , hourRate, workPercent, readgroup
                    );
            }
    ...
    public onFormSubmit(event, index) {

...
export class Editor implements IEditor {
    ...
    public startEdit(value:any, cell: IJQItem) {
        if (!this.inputBox) {
    ...
    public finalizeEdit(cancel = false) {
        if (!(this.editEvents && this.editEvents.finalizeEdit)) {
            return;
        }
        var txt = this.inputBox.val();
        if (txt !== this.originalValue) {
            this.editEvents.finalizeEdit(txt, this.position);
        }
    }

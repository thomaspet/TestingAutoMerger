...
import {DistributionPlan, DistributionPlanElementType, CompanySettings, Distributions, DistributionType} from '../../../unientities';
...
export class UniDistributionSettings {
    ...
    tabs = [
        ...
        {name: 'Ordre', value: 'order'},
        {name: 'Tilbud', value: 'quote'},
        {name: 'Purring', value: 'reminder', featurePermission: 'ui.debt-collection'},
    ];
    ...
    public ngOnInit() {
        ...
        this.hideDistributionInfo = JSON.parse(localStorage.getItem('hideDistributionInfo'));
    ...
    public getDistributionPlans() {
        ...
        Observable.forkJoin([
            ...
            this.companySettingsService.Get(1, ['Distributions', 'APOutgoing', 'APIncomming'])
        ]).subscribe(([plans, companySettings]) => {
            this.plans = plans;
            ...
            this.distributionPlanService.getCustomerCount(this.plans).subscribe((response) => {
                ...
                this.plans = this.plans.map(plan => {
                    ...
                    */
                    plan.Elements.map(e => {
                        e.StatusCode = e.ElementType.Name === 'Email' ? 30001 : null;
                    });
    ...
    onActionClick(actionName: string, plan: DistributionPlan, type: any) {
        switch (actionName) {
            case 'standard':
    ...
    openPlanModal(plan: DistributionPlan, type) {
        this.getPlanModal(plan, type).subscribe((response) => {
            if (response) {
    ...
    hideInfo() {
        this.hideDistributionInfo = true;
        localStorage.setItem('hideDistributionInfo', JSON.stringify(this.hideDistributionInfo));
    }

    getPlanModal(plan: DistributionPlan, type) {
        if (!plan) {
            plan = new DistributionPlan();
            plan.Name = 'Standard utsending av ' + type.label.toLowerCase();
            plan.EntityType = type.value;
            plan.ID = 0;
            plan.Elements = [];
        }
        const header = 'Utsendelsesplan for ' + type.label.toLowerCase();
        const data = {
            plan: plan,
            currentType: type,
        ...
        return this.modalService.open(DistributionPlanModal, { data: data, header: header }).onClose;
    }

    updateTabAndUrl() {
        this.type = this.entityTypes[this.activeIndex];
    }
    ...
    public setNewStandard(plan: any, type: any, turnedOnAutoDistribute: boolean = false) {
        ...
        this.companySettingsService.Put(1, this.companySettings).subscribe((settings) => {
            ...
            turnedOnAutoDistribute ? 'Automatisk utsendelse av faktura ble også aktivert' : '');
            this.getDistributionPlans();
        });
    }
    ...
    public turnOnOffAutomaticDistribution() {
        ...
        // Cant turn on automatic distribution without a standard plan for sending
        if (this.active && !this.entityTypes[0].defaultPlan) {
            this.getPlanModal(null, this.entityTypes[0]).subscribe((response) => {
                if (response?.setAsDefault) {
                    this.companySettings.AutoDistributeInvoice = this.active;
                    this.setNewStandard(response.plan, this.entityTypes[0], true);
                } else if (response) {
                    this.active = false;
                    this.getDistributionPlans();
                } else {

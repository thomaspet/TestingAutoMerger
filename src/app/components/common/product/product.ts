...
const CHILD_ROUTES = [
    new AsyncRoute({
        ...
        path: '/list',
        name: 'ProductList',
        loader: () => ComponentProxy.LoadComponentAsync("ProductList", "src/app/components/common/product/list/productList")
    }),
    
    new AsyncRoute({    
        path: '/details/:id',
        name: 'Details',
        loader: () => ComponentProxy.LoadComponentAsync("ProductDetails", "src/app/components/common/product/details/productDetails")
    })       
]
@Component({
    selector: "product",
